  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 bg-card border-b border-border px-4 py-3">
        <button onClick={() => router.back()} className="flex items-center gap-1 text-sm text-muted-foreground mb-1">
          <ChevronLeft className="w-4 h-4" /> Volver
        </button>
        <h1 className="text-lg font-bold text-foreground">Introducir resultado</h1>
      </header>
        ...
        {sets.map((set, i) => (
          <div key={i} className="flex flex-col gap-2">
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <Minus className="w-6 h-6 text-muted-foreground mt-4" />
              <div className="flex flex-col items-center gap-1">
